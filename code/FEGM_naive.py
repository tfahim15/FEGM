...
def generate_orders(code):
    ret = []
    perm = permutations([i for i in range(len(code))])
    for p in list(perm):
        n_c = []
        for i in range(len(p)):
            n_c.append((code[i][0], code[i][1], code[i][2], code[i][3], code[i][4], p[i]))
        ret.append(n_c)
    return ret
...
def GSpan(code, graphs, min_sup):
    ...
    for key in extentions:
        ...
        if not isCannonical(new_code):
            continue
        if sup >= min_sup:
            for g in generate_orders(new_code):
                candidates += 1
                s = 0
                for G in graphs:
                    if len(subgraphIsomorphisms_ordered(g, G)) > 0:
                        s += 1
                if s >= min_sup:
                    freq += 1
                    print(g)
...
open("FEGM_naive.txt", "a").write(str(time.time()-t)+" "+str(base_sup)+" "+str(candidates)+" "+str(freq)+" "+str(file_name)+"\n")